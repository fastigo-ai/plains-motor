...
export const createCheckoutSession = async (req, res) => {
    ...
    try {
      ...
      // Create Stripe Checkout Session with proper redirects
      const checkoutSession = await stripe.checkout.sessions.create({
        payment_method_types: ['card'],
        line_items: [
          {
            price_data: {
              ...
              unit_amount: Math.floor(totalAmount) * 100, // Amount in cents, floored
            },
            quantity: 1,
          },
        ],
        mode: 'payment',
        // Success URL - redirect to success page
        ...
        // Cancel URL - redirect back to booking confirmation page
        cancel_url: `${baseUrl}/booking-cancel?session_id={CHECKOUT_SESSION_ID}&action=${bookingAction}&booking_id=${savedBooking._id}&property_id=${propertyId}`,
        metadata: {
      ...
      res.status(201).json({
        ...
        redirectUrls: {
          ...
          return: `${baseUrl}/confirm?property_id=${propertyId}&check_in=${checkInDate}&check_out=${checkOutDate}&guests=${JSON.stringify(guests)}`
        },
        bookingDetails: {
      ...
      if (error.code === 11000) {
        ...
        if (error.keyPattern?.property || error.keyPattern?.checkInDate) {
          return res.status(400).json({
            ...
            redirectUrl: `${process.env.CLIENT_URL || 'http://localhost:3000'}/properties`
          });
        }
